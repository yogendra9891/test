...
// define route
var route = new Route('post', '/getShippingMathods');
...
route.use(function(req, res, next) {
  ...
  MasterData.getShippingMathods(store_id, cart_id, function(err,
    shipingMathods) {
    if (err) {
      magentoLoginError.handleError(err, req, res, next,
        api_errors.get_shipping_methods.error_code,
        err.faultString, false);
    } else {
      data = {
        "shippingMathods": shipingMathods
      };
