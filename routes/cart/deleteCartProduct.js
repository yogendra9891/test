...
// define route
var route = new Route('post', '/deleteCartProduct');
...
route.use(function(req, res, next) {
  ...
  Product.removeCartProduct(store_id, cart_id, products, function(err, resp) {
    if (err) {
      magentoLoginError.handleError(err, req, res, next,
        api_errors.remove_cart_product.error_code,
